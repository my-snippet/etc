# 1 cont(without comp), 2 comp

ans_file="ans_$(date +"%T").txt"
controller_ip="10.0.0.10"

compute_ip_prefix="10.0.1"
compute_ips=("$compute_ip_prefix.10" "$compute_ip_prefix.11")
...
sed -i "s/CONFIG_COMPUTE_HOSTS=.*/CONFIG_COMPUTE_HOSTS=${compute_ips[0]},${compute_ips[1]}/g" $ans_file
...
sed -i "s/CONFIG_PROVISION_OVS_BRIDGE=.*/CONFIG_PROVISION_OVS_BRIDGE=n/g" $ans_file

# Etc
sed -i "s/CONFIG_NOVA_NETWORK_PUBIF=.*/CONFIG_NOVA_NETWORK_PUBIF=enp0s3/g" $ans_file
